...
| 文件 | 内容 |
| --- | --- |
| `subset_token_stats.csv` | 每个子集一行：样本数、原始长度分布、首次截断数量及比例；启用 `--packing` 后增加各子集后续损失及 token 占比变化。 |
| `packing_token_stats.csv` | 启用 `--packing` 时生成：打包后有效长度分布、裁切、整条丢弃、label shift 和 padding。 |
